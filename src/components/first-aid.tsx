...
export function FirstAid({ title, description, firstAidItems }: FirstAidProps) {
    ...
    return (
        <div className="h-screen flex flex-col space-y-4">
            <div>
            <iframe 
                src={selectedLink.includes("youtube") ? selectedLink.replace('watch?v=', 'embed/') + '?autoplay=1&mute=1' : selectedLink} 
                className="w-full h-48 sm:h-80 rounded-2xl md:h-[500px]" 
                allowFullScreen
            ></iframe>
            </div>
